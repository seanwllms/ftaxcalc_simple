...
function(input, output, session) {
  ...
  tpdata <- reactiveValues()
  tpdata$status <- "Married Filing Jointly"
  tpdata$dependents <- 0
  tpdata$child <- 0
  tpdata$agi <- reactiveVal(0)
  tpdata$capgains <- 0
  tpdata$med <- 0
  tpdata$intpd  <- 0 
  tpdata$state <- 0
...
# 
  ...
  #   tpdata$agi() <- 40000
  #   tpdata$intpd <- 0
  #   tpdata$statetx_ded <- 0
  #   tpdata$realest_ded <- 0
  #   tpdata$pptax_ded <- 0
  #   tpdata$othtax_ded <- 0
  #   tpdata$intpd_ded <- 0
  #   tpdata$char_ded <- 0
  ...
  calctax <- reactive({
      ...
      tot_item_base <- tpdata$med + tpdata$intpd + tpdata$state + tpdata$realest +
      tpdata$pptax + tpdata$othtax + tpdata$char + tpdata$jobmisc + tpdata$othermisc + tpdata$caustheft

    })
  
  
  
  output$agi <- renderText(tpdata$agi())
# 
...
#     pease_agi <- filter(pease_limits, status == tpdata$status) %>%
#       pull(limit)
# 
#     pease_base <- max(min(.03*(tpdata$agi-pease_agi), .8*tot_item_base),0)
# 
#     st_ded_base <- filter(stded, status ==tpdata$status, basealt == "Base") %>%
#       pull(standard_deduction)
# 
#     deductions_claimed_base <- max(st_ded_base, tot_item_base-pease_base)
# 
#     personal_exemptions <- ifelse(tpdata$status == "Married Filing Jointly", 2, 1)*4150
# 
#     exemptions_base <- 4150 * tpdata$dependents + personal_exemptions
# 
#     exemptions_phaseout_base <- ceiling((tpdata$agi-pease_agi)/2500)*.02*exemptions_base
# 
...
#                                tpdata$agi - max(st_ded_base, tot_item_base-pease_base) - exemptions_allowed)
# 
#     capgains_tax_base <- max(0,
#                              tpdata$capgains * capgains_rate(tpdata$status, taxable_income_base,"Base"))
# 
#     tax_base <- max(0,
#                     calculate_tax(tpdata$status, taxable_income_base-tpdata$capgains, "Base"))
# 
#     cc_phase_floor_base <- filter(childcr, status==tpdata$status, basealt == "Base") %>%
#       pull(ccred_phaseout)
# 
#     child_phaseout_base <- ceiling((max(0,as.numeric(tpdata$agi) - cc_phase_floor_base))/1000)*50
# 
#     child_credit_base <- max(0,tpdata$child * 1000 - child_phaseout_base)
# 
...
# 
#     #SALT data_alt
#     salt_limit <- ifelse(tpdata$status == "Married Filing Separately", 5000, 10000)
# 
#     salt_alt <- min(salt_limit, tpdata$state + tpdata$realest + tpdata$pptax + tpdata$othtax)
# 
#     tot_item_alt <- tpdata$med + tpdata$intpd + salt_alt + tpdata$char
# 
#     st_ded_alt <- filter(stded, status ==tpdata$status, basealt == "Alt") %>%
#       pull(standard_deduction)
# 
...
#                               tpdata$agi - max(st_ded_alt, tot_item_alt))
# 
#     capgains_tax_alt <- max(0,
#                             tpdata$capgains * capgains_rate(tpdata$status, taxable_income_alt,"Alt"))
# 
#     tax_alt <- max(0,
#                    calculate_tax(tpdata$status, taxable_income_alt-tpdata$capgains, "Alt"))
# 
#     cc_phase_floor_alt <- filter(childcr, status==tpdata$status, basealt == "Alt") %>%
#       pull(ccred_phaseout)
# 
#     child_phaseout_alt <- ceiling((max(0,as.numeric(tpdata$agi) - cc_phase_floor_alt))/1000)*50
# 
#     otherdepcred <- ifelse(tpdata$child == tpdata$dependents, 0,
#                            ifelse(tpdata$child < tpdata$dependents, 500*(tpdata$dependents-tpdata$child), 0))
# 
#     child_credit_alt <- max(0,(otherdepcred+tpdata$child * 2000) - child_phaseout_alt)
# 
...
#                               ifelse(deductions_claimed_base == st_ded_base, 0, tpdata$state)
#     )
# 
...
#       filter(status == tpdata$status, basealt == "Base") %>%
#       pull(id_lim)
# 
...
#     } else {
#       #limit i.d. to limits
#       disallowed_id_add_base <- min(.03*(tpdata$agi - state_id_limit_base),
#                                .8*(tot_item_base- tpdata$med - tpdata$intpd - tpdata$caustheft))
# 
...
# 
#     #get phaseout rate
#     phaseoutrate <- ifelse(tpdata$status == "Married Filing Separately", 1250, 2500)
# 
#     #calculate disallowed p.e.
#     disallowed_pe_add_base <- exemptions_base*ceiling((tpdata$agi-state_pe_limit_base)/phaseoutrate)*.02
# 
...
# 
#     #calculate MN tax.
#     mn_tax_base <- calculate_mntax(tpdata$status, mti_base, "Base")
# 
# 
...
# 
# ####calculate state taxes add-back####
#     income_deducted <- min(min(salt_limit, tpdata$state),
#                            salt_limit - tpdata$pptax -tpdata$othtax - tpdata$realest)
# 
...
#       filter(status == tpdata$status, basealt == "Alt") %>%
#       pull(id_lim)
# 
#     #only add back i.d. if itemizing
...
#     } else {
#     #limit i.d. to limits
#       disallowed_id_add_alt <- min(.03*(tpdata$agi - state_id_limit_alt),
#                                   .8*(tot_item_alt - tpdata$med - tpdata$intpd - tpdata$caustheft))
#     #don't add back negative numbers
...
# 
# 
#     mn_tax_alt <- calculate_mntax(tpdata$status, mti_alt, "Alt")
# 
